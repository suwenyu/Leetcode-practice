...
public:
    int mincostTickets(vector<int>& days, vector<int>& costs) {
        ...
        for(int i = 1 ; i<=365 ; i++){
            it = find(days.begin(), days.end(), i);
            if(it == days.end()) tmp[i] = tmp[i-1];
            else{
                tmp[i] = min({tmp[i-1]+costs[0], tmp[max(0, i - 7)]+costs[1], tmp[max(0, i-30)]+ costs[2]});
            }
        }

        return tmp[days[days.size()-1]];
    }
};
```
